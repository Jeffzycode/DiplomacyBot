...
module.exports = {    
    ...
    async execute(message, args){
        ...
        if(channelProfile.Item === undefined) {//Not a diplo channel
            await message.channel.send("You cannot set a clause in a non-diplo channel.");
            return;
        }
        if(Object.keys(channelProfile.Item.curTreaty).length === 0) {//There is no treaty in the channel
            await message.channel.send("You cannot set a clause of a non-existent treaty. To make a new treaty, use " + process.env.PFIX + "make-treaty.");
            return;
        }
        ...
        if(channelProfile.Item.curTreaty.clauses.length < clauseNumber) {//Trying to set a non-existent clause
            await message.channel.send("You cannot set a non-existent clause.");
            return;
        }
        //Set changes to DB
        ...
        await message.channel.send("Successfully set clause no. " + clauseNumber);
    }
}