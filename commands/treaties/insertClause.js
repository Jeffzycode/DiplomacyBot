...
module.exports = {    
    ...
    async execute(message, args){
        ...
        if(channelProfile.Item === undefined) {//Not a diplo channel
            message.channel.send("You cannot insert a clause in a non-diplo channel.");
            return;
        }
        if(Object.keys(channelProfile.Item.curTreaty).length === 0) {//There is no treaty in the channel
            message.channel.send("You cannot insert a clause into a non-existent treaty. To make a new treaty, use ~make-treaty.");
            return;
        }
        ...
        if(channelProfile.Item.curTreaty.clauses.length < clauseNumber) {//Trying to insert into an invalid position
            message.channel.send("You cannot insert a clause into an invalid position.");
            return;
        }
        //Set changes to DB
        ...
        if(channelProfile.Item.curTreaty.clauses.length > process.env.MAX_CLAUSES) {
            message.channel.send("There can be a maximum of " + process.env.MAX_CLAUSES + " clauses in a single treaty.");
            return;
        }
        ...
        message.channel.send("Successfully inserted a clause into position no. " + clauseNumber);
    }
}